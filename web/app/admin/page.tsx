...
export default async function AdminOverview() {
  ...
  if (stats.pkg.pending > 0)
    alerts.push({
      text: `${stats.pkg.pending} PKGs pending review`,
      type: "warning",
      href: "/admin/queue",
    });
  if (stats.sources.dead > 0)
    alerts.push({
      text: `${stats.sources.dead} dead sources need fixing`,
      type: "error",
      href: "/admin/sources?status=dead",
    });
  if (stats.reports.open > 0)
    alerts.push({
      text: `${stats.reports.open} open link reports`,
      type: "warning",
      href: "/admin/sources?tab=reports",
    });
    ...
    <div className="animate-fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        <QuickAction
          ...
          label="Review Queue"
          desc={`${stats.pkg.pending} pending`}
        />
        <QuickAction
          href="/admin/pkgs"
          icon={<IconCatalog size={24} />}
          label="Manage PKGs"
          desc="Add, edit, remove"
        />
        <QuickAction
          href="/admin/sources"
          icon={<IconLink size={24} />}
          label="Link Health"
          desc="Check & fix sources"
        />
      </div>
    </div>
  );
