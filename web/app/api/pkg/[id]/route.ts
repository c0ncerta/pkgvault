...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  ...
  // Update the PKG and (optionally) its game atomically.
  const updated = await db.transaction(async (tx) => {
    ...
    if (data.game) {
      ...
      if (Object.keys(gameData).length > 0) {
        ...
        if (file.gameId) {
          await tx.update(games).set(gameData).where(eq(games.id, file.gameId));
          ...
          if (newGame) {
            await tx.update(pkgFiles).set({ gameId: newGame.id }).where(eq(pkgFiles.id, id));
          }
        }
      }
    }
