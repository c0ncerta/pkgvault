...
export async function POST(request: NextRequest) {
  ...
  // Unset other primaries + insert atomically so we never end up with two
  // primaries (or none) if the insert fails.
  const [source] = await db.transaction(async (tx) => {
    if (isPrimary) {
      await tx.update(pkgSources).set({ isPrimary: false }).where(eq(pkgSources.pkgId, pkgId));
    }
