...
import { pkgFiles, forumPosts, games } from "@/db/schema";
...
function formatBytes(b: bigint | number) {
  const n = Number(b);
...
export default async function ProfilePage() {
  ...
  let pkgs: Array<{
    id: string; title: string; version: string | null; sizeBytes: bigint;
    status: string; downloadCount: number; createdAt: Date;
    gamePlatform: string | null;
  }> = [];
  let posts: Array<{
    id: string; bodyMd: string; score: number; createdAt: Date;
  ...
  try {
    ...
    pkgs = await db
      .select({
        id: pkgFiles.id, title: pkgFiles.title, version: pkgFiles.version,
        sizeBytes: pkgFiles.sizeBytes, status: pkgFiles.status,
    ...
    posts = await db
      .select({
        ...
        score: forumPosts.score, createdAt: forumPosts.createdAt,
  ...
  const serializedPkgs = pkgs.map((p) => ({
    ...p, sizeBytes: Number(p.sizeBytes), createdAt: p.createdAt.toISOString(),
  }));
  ...
  return (
    <div className="animate-fade-in responsive-grid-2" style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 28 }}>
      {/* Profile card */}
      <GlassCard padding="24px" style={{ alignSelf: "start" }}>
        <div style={{
          ...
          fontWeight: 800, fontSize: "1.5rem", color: "#fff", marginBottom: 14,
        }}>{initials}</div>
        <h2 style={{ fontSize: "1.25rem", fontWeight: 800, color: "var(--color-text-primary)" }}>{userName}</h2>
        <p style={{ fontSize: "0.75rem", color: "var(--color-text-muted)", fontFamily: "var(--font-mono)", marginBottom: 16 }}>
          member since · {joined}
        </p>
        ...
        <SpecList items={[
          { label: "contributions", value: `${stats.pkgsApproved} ✓ · ${stats.pkgsPending} pending` },
          { label: "forum posts", value: String(stats.forumPosts) },
          { label: "total pkgs", value: String(stats.pkgsTotal) },
        ]} />
        <div style={{ height: 1, background: "rgba(255,255,255,0.06)", margin: "16px 0" }} />
        <div style={{ display: "flex", gap: 8 }}>
          <a href="/settings" className="btn-secondary" style={{ flex: 1, textAlign: "center", textDecoration: "none" }}>Edit profile</a>
        </div>
      </GlassCard>
