...
import { IconFire, IconCatalog, IconCheck, IconQueue } from "@/components/ui/icons";
...
export default async function CatalogPage({ searchParams }: CatalogPageProps) {
  ...
  let items: Array<{
    ...
    createdAt: Date; gameTitle: string | null;
  ...
  try {
    ...
    const orderBy = sort === "oldest" ? asc(pkgFiles.createdAt)
      : sort === "downloads" ? desc(pkgFiles.downloadCount)
      : sort === "title" ? asc(pkgFiles.title)
    ...
    const [data, countResult, statsResult, platformAgg, regionAgg] = await Promise.all([
      db
        .select({
          ...
          sha256: pkgFiles.sha256, sizeBytes: pkgFiles.sizeBytes,
          version: pkgFiles.version, fwRequired: pkgFiles.fwRequired,
          downloadCount: pkgFiles.downloadCount, createdAt: pkgFiles.createdAt,
          gameTitle: games.title, gamePlatform: games.platform,
      ...
      }).from(pkgFiles).leftJoin(games, eq(pkgFiles.gameId, games.id)).where(sql`${pkgFiles.deletedAt} IS NULL`),
      db.select({
    ...
    platformCounts = platformAgg.filter((p): p is { value: string; count: number } => p.value !== null);
  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 24px 64px", position: "relative", zIndex: 1 }}>
        <div className="animate-fade-in" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 12 }}>
          <div>
          ...
          <Link href="/upload" className="btn-primary">＋ Upload PKG</Link>
        </div>

        {/* Stats strip */}
        <div className="animate-fade-in delay-1 responsive-stats" style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            <GlassCard key={i} padding="14px 18px">
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "var(--color-text-primary)", lineHeight: 1 }}>{s.v}</div>
                <div style={{ fontSize: "0.7rem", color: "var(--color-text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: 4, fontWeight: 500 }}>{s.l}</div>
              </div>
            </GlassCard>
          ))}
        </div>
        ...
        <div className="responsive-catalog-layout" style={{ display: "grid", gridTemplateColumns: "260px minmax(0, 1fr)", gap: 22 }}>
          <CatalogFilters
            currentQ={q} currentSort={sort} currentPlatform={platform}
            platformCounts={platformCounts} regionCounts={regionCounts} total={total}
          />

          <div>
        {/* Grid */}
        {items.length > 0 ? (
          <div className="responsive-grid-cards" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {items.map((item, i) => (
              <Link
                key={item.id}
              ...
              >
                <GlassCard padding="0" style={{ overflow: "hidden", display: "flex", flexDirection: "column", height: "100%" }}>
                  <div style={{
                    ...
                    {item.downloadCount > 50 && (
                      <span style={{
                        ...
                        <IconFire size={12} style={{ display: "inline", verticalAlign: "middle" }} /> hot
                      </span>
                    )}
                  </div>
                  <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 6 }}>
                    <span style={{ fontSize: "0.95rem", fontWeight: 600, color: "var(--color-text-primary)" }}>{item.title}</span>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                      {item.gamePlatform && <Tag>{item.gamePlatform}</Tag>}
                      {item.gameRegion && <Tag>{item.gameRegion}</Tag>}
                      {item.version && (
                        <span style={{ marginLeft: "auto", fontSize: "0.7rem", color: "var(--color-text-muted)", fontFamily: "var(--font-mono)" }}>{item.version}</span>
                      )}
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
                      ...
                      <span style={{
                        ...
                        color: item.sha256 !== "pending" ? "var(--color-success)" : "var(--color-warning)",
                        display: "inline-flex", alignItems: "center", gap: 3,
                      }}>
                        {item.sha256 !== "pending" ? <><IconCheck size={11} /> verified</> : <><IconQueue size={11} /> pending</>}
                      </span>
                    </div>
                  </div>
                </GlassCard>
          ...
          <GlassCard padding="80px 40px" className="animate-fade-in" style={{ textAlign: "center" }}>
            <div style={{ fontSize: "3rem", marginBottom: 16, opacity: 0.3 }}><IconCatalog size={48} /></div>
            <h2 style={{ fontSize: "1.25rem", fontWeight: 700, color: "var(--color-text-primary)", marginBottom: 8 }}>
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 32 }}>
            {page > 1 && (
              <Link href={`/catalog?${new URLSearchParams({ ...(q && { q }), ...(sort !== "newest" && { sort }), page: String(page - 1) })}`} className="btn-secondary">
                ← Previous
              </Link>
            )}
            <span style={{ display: "flex", alignItems: "center", fontSize: "0.85rem", color: "var(--color-text-muted)", padding: "0 16px" }}>
