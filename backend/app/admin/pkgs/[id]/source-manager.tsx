...
interface Source {
  ...
  downloadCount: number; lastCheckedAt: string | null; notes: string | null;
  createdAt: string;
}

export function PkgSourceManager({ pkgId, sources: initialSources }: { pkgId: string; sources: Source[] }) {
  ...
  return (
    <GlassCard padding="20px 24px">
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 10, alignItems: "center" }}>
            <input
              ...
              style={inputStyle}
            />
            <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.8rem", color: "var(--color-text-secondary)", cursor: "pointer", whiteSpace: "nowrap" }}>
      ...
      {/* Sources list */}
      {sources.length === 0 ? (
        <div style={{ padding: "30px 0", textAlign: "center", color: "#475569", fontSize: "0.85rem" }}>
          No sources yet. Add a download link above.
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {sources.map(source => {
            const st = statusConfig[source.status] ?? statusConfig['unknown'] ?? { color: '#64748b', label: 'Unknown' };
            return (
              <div key={source.id} style={{
                ...
                <div style={{
                  ...
                  background: st.color, boxShadow: `0 0 6px ${st.color}40`,
                }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {source.isPrimary && (
                      <span style={{ fontSize: "0.6rem", padding: "1px 5px", borderRadius: 4, color: "#818cf8", background: "rgba(99,102,241,0.12)", fontWeight: 600 }}>
                        PRIMARY
                      </span>
                    )}
                    {source.label && <span style={{ fontSize: "0.7rem", color: "#475569" }}>• {source.label}</span>}
                  </div>
                  <div style={{
                    fontSize: "0.72rem", color: "#475569", fontFamily: "var(--font-mono)",
                ...
                <LiquidButton variant="ghost" size="sm" onClick={() => handleCheck(source.id)} title="Check status"><IconSearch size={14} /></LiquidButton>
                <LiquidButton variant="danger" size="sm" onClick={() => handleDelete(source.id)} title="Delete">✗</LiquidButton>
              </div>
            );
          })}
        </div>
