...
import { IconCatalog, IconUser, IconSettings } from "@/components/ui/icons";
...
export default async function QueuePage() {
  let pendingPkgs: {
    ...
    fwRequired: string | null; createdAt: Date;
  ...
  return (
    <div className="animate-fade-in">
      ...
      {pendingPkgs.length === 0 ? (
        <GlassCard variant="content" cornerRadius={16} padding="60px 20px" style={{ textAlign: "center" }}>
          <div style={{ fontSize: "2.5rem", marginBottom: 12, opacity: 0.3 }}>✓</div>
          <div style={{ color: "#34d399", fontWeight: 600, fontSize: "1.1rem" }}>Queue is clear</div>
          <div style={{ color: "#475569", fontSize: "0.85rem", marginTop: 4 }}>No pending submissions</div>
        </GlassCard>
      ) : (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    ...
                    {pkg.version && (
                      <span style={{ fontSize: "0.75rem", color: "#475569", fontFamily: "var(--font-mono)" }}>
                        v{pkg.version}
                      </span>
                    )}
                  </div>
                  ...
                  {pkg.description && (
                    <p style={{ color: "var(--color-text-secondary)", fontSize: "0.85rem", marginBottom: 10, lineHeight: 1.5 }}>
                      {pkg.description.slice(0, 200)}{pkg.description.length > 200 ? "..." : ""}
                    </p>
                  )}

                  <div style={{ display: "flex", gap: 20, fontSize: "0.75rem", color: "#475569" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><IconCatalog size={12} /> {formatBytes(pkg.sizeBytes)}</span>
                    {pkg.gameTitle && <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><IconSettings size={12} /> {pkg.gameTitle}</span>}
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><IconUser size={12} /> {pkg.uploaderName ?? "anonymous"}</span>
                    <span>{pkg.createdAt.toISOString().slice(0, 10)}</span>
                    {pkg.fwRequired && <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><IconSettings size={12} /> FW {pkg.fwRequired}</span>}
                  </div>

                  <div style={{
                    marginTop: 10, fontSize: "0.7rem", color: "#475569",
