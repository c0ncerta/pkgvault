...
const endpoints = [
  { method: "GET", path: "/api/catalog", desc: "List all approved PKGs with pagination and filters", auth: false },
  { method: "GET", path: "/api/catalog/:id", desc: "Get PKG details including download sources", auth: false },
  { method: "POST", path: "/api/upload", desc: "Upload a new PKG file for review", auth: true },
  { method: "GET", path: "/api/forum/threads", desc: "List forum threads with pagination", auth: false },
  ...
  { method: "PATCH", path: "/api/admin/pkgs/:id/status", desc: "Approve or reject a PKG", auth: true },
];
  ...
  return (
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "48px 24px 64px" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 32 }}>
        <span className="tag" style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem" }}>Base URL: /api</span>
        <span className="tag" style={{ fontFamily: "var(--font-mono)", fontSize: "0.7rem" }}>Content-Type: application/json</span>
      </div>
        ...
        {endpoints.map((ep, i) => (
          <GlassCard key={i} padding="14px 18px">
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              {ep.auth && (
                <span className="tag tag-warning" style={{ fontSize: "0.6rem", padding: "1px 6px" }}>AUTH</span>
              )}
            </div>
            <p style={{
              fontSize: "0.78rem", color: "var(--color-text-muted)",
              margin: "6px 0 0 56px", lineHeight: 1.4,
            }}>
              {ep.desc}
            </p>
          </GlassCard>
